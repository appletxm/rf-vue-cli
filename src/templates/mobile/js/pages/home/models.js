...
import { SET_USER_DEFAULT_ADDRESS, SET_LOGIN_METHOD, SET_USER_LOGIN_STATUS } from 'store/mutationTypes'
...
export default {
    ...
    axios.post(apiUrls.getDefaultAddress, params)
      .then((res) => {
        ...
        if (!defaultAddress && res.data.length !== 0) {
          _this.defaultAddress = this.checkNullFileds((res.data)[0], 1)
  ...
  checkNullFileds(info, addressType) {
    if (addressType === 1) {
      info.cityName = info.cityName || '广州市'
      info.areaName = info.areaName || '花都区'
      info.provinceName = info.provinceName || '广东省'
      info.provinceCode = info.provinceName || '440000'
      info.cityCode = info.provinceName || '440000'
      info.areaCode = info.provinceName || '440114'
    }
